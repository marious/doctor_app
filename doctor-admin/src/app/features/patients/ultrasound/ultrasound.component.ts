...
import { UltrasoundFinding } from '../../../core/models';
...
@Component({
  ...
  styleUrls: ['./ultrasound.component.scss'],
})
export class UltrasoundComponent implements OnInit {
  ...
  findings = signal<UltrasoundFinding[]>([]);
  loading = signal(true);
  ...
  load() {
    this.svc.ultrasoundFindings(this.patientId).subscribe({
      next: res => { this.findings.set(res.data); this.loading.set(false); },
  ...
  submit() {
    ...
    this.svc.storeUltrasound(this.patientId, fd).subscribe({
  ...
  doDelete() {
    ...
    if (!id) return;
    this.svc.deleteUltrasound(this.patientId, id).subscribe({ next: () => { this.deleteTarget.set(null); this.load(); } });
  }
}
